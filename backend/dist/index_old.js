...
// User endpoints
app.get("/users", authMiddleware, requireRole(['HOST', 'ADMIN']), async (_req, res) => {
    try {
...
app.post("/users", authMiddleware, requireRole(['HOST']), async (req, res) => {
...
app.put("/users/:id", authMiddleware, requireRole(['HOST']), async (req, res) => {
...
app.delete("/users/:id", authMiddleware, requireRole(['HOST']), async (req, res) => {
    const userId = parseInt(req.params.id || '');
    try {
        const currentUser = await prisma.user.findUnique({ where: { id: userId } });
        if (!currentUser) {
...
app.delete('/notifications/bulk', authMiddleware, async (req, res) => {
...
app.get('/service-categories', authMiddleware, async (_req, res) => {
    try {
